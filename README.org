...
** Terminal
*** term.el
    #+BEGIN_SRC emacs-lisp :tangle yes :noweb tes
      (use-package term
          :defines (term-raw-map)
          :config
          (defconst ayrc/term-function-key-alist '((f1  . "\e[OP")
                                                   (f2  . "\e[OQ")
                                                   (f3  . "\e[OR")
                                                   (f4  . "\e[OS")
                                                   (f5  . "\e[15~")
          ...
          ;;         (read-kbd-macro (format "<%s>" 'f10))

          ;;         (let ((code            "\e[21~"))
          ;;             (lambda () (interactive) (term-send-raw-string code)))))

          ;; (dolist (spec ayrc/term-function-key-alist)
          ;;     (define-key term-raw-map
          ;;         (read-kbd-macro (format "<%s>" (car spec)))

          ;;         (let* ((lexical-binding t)
          ;;                (code            (cdr spec)))
          ;;             (lambda () (interactive) (term-send-raw-string (cdr spec))))))
          )
   #+END_SRC
